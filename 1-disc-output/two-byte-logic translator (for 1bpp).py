        ...
        with open(tbl_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if "=" in line:
                    ...
                    translation_map[key.strip().upper()] = value.strip()
                elif line.startswith("*") or line.startswith("/"):
                    # Handle special cases if needed
                    translation_map[line[1:].strip().upper()] = f"[{line[0]}Logic]"
...
def translate_hex_file(hex_file, tbl_map, output_file):
    try:
        ...
        while i < len(hex_data):
            # Check for two-byte lookup cases
            if hex_data[i:i+2].upper() in {"17", "1E", "1F"} and i + 4 <= len(hex_data):
                two_byte_key = hex_data[i:i+4].upper()  # Combine current and next byte
                if two_byte_key in tbl_map:
                    translated_text.append(tbl_map[two_byte_key])
                    i += 4  # Move past both bytes
                    continue
                else:
                    translated_text.append(f"[{two_byte_key}]")
                    i += 4
                    continue

            # Fallback to one-byte lookup
