...
def salvage_rom(original_rom, edited_rom, output_rom, offsets):
    ...
    for character, info in offsets.items():
        ...
        battle_sprite_data = read_bytes(edited_rom, battle_sprites_start, 28672) # 64 tiles of 32 bytes * 14 characters
        write_bytes(output_rom, battle_sprites_start, battle_sprite_data) 

        # Read & write 32 bytes of battle palette data
        palette_data = read_bytes(edited_rom, pal_start, 32) # 16 colors of 2 bytes  * 14 characters
        write_bytes(output_rom, pal_start, palette_data)    

        # Read & write 16 tiles of portrait sprite data (256 bytes)
        portrait_sprite_data = read_bytes(edited_rom, portrait_start, 3584) # 256 * 14 characters 
        write_bytes(output_rom, portrait_start, portrait_sprite_data)

        # Read & write 8 colors (16 bytes) of portrait palette data
        portrait_pal_data = read_bytes(edited_rom, portrait_pal_start, 224) # 16 * 14 characters
        write_bytes(output_rom, portrait_pal_start, portrait_pal_data)

        # Read & write 48 tiles of map sprite data
        map_sprite_data = read_bytes(edited_rom, map_sprites_start, 1536) # 48 tiles of 32 bytes * 14 characters
        write_bytes(output_rom, map_sprites_start, map_sprite_data)

        # Read & write 1 byte palette index
        map_pal_data = read_bytes(edited_rom, map_pal_index, 14) # all 14 bytes of palette indices
        ...
        # Console test prints
        print(f"{character}")
        print(f"  All Sprite Data: {battle_sprite_data.hex()}")
        print(f"  14 Palettes' Data: {palette_data.hex()}")
        print(f"  14 Portrait Sprites' Data: {portrait_sprite_data.hex()}")
        print(f"  14 Portrait Palettes: {portrait_pal_data.hex()}")
        print(f"  14 Map Sprites' Data: {map_sprite_data.hex()}")
        print(f"  14 Map Palette Indices: {map_pal_data.hex()}")
...
with open('full_cast_offset_data.json', 'r') as f:
    offsets = json.load(f)
